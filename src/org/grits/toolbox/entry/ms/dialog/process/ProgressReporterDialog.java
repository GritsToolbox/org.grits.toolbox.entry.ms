package org.grits.toolbox.entry.ms.dialog.process;
...
import org.grits.toolbox.tools.spectrafiltering.dialog.process.IProgressReporter;
...
public class ProgressReporterDialog extends Dialog implements IProgressReporter {
	...
	/** Label that shows current task on the main progress bar */
		...
		if (!isCanceled) {
			...
			this.display.syncExec(new Runnable() {
				...
				public void run() {
					...
					if (m_textNotification != null && successful) {
	...
	private void createNotificationText() {
		...
		// Move the Text above the button
	...
	/**
	 * This area is only for errors. The text will be created when this method is called.
	 *
	 * @param description
	 */
	@Override
