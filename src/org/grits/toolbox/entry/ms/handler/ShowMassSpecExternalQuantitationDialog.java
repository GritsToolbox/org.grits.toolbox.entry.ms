 ...
 *
 */
public class ShowMassSpecExternalQuantitationDialog {
	private static final Logger logger = Logger.getLogger(ShowMassSpecExternalQuantitationDialog.class);
	...
	public Object execute(@Named(IServiceConstants.ACTIVE_PART) MPart part,
		...
		if( msProp instanceof MassSpecEntityProperty ) {
			MassSpecExternalQuantDialog mseqd = new MassSpecExternalQuantDialog(shell, curView);
			mseqd.addListener(curView);
			mseqd.open();
			
		} else {
			logger.error("An MS-related report must be open and active in order to apply external quantitation");
			ErrorUtils.createWarningMessageBox(shell, "No open report", "An MS-related report must be open and active in order to apply external quantitation");			
		}
		return null;
